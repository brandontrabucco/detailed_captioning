...
def _mask_and_slice(x):
    image_id, caption, image_features, object_features = x["image_id"], x["caption"], x["image_features"], x["object_features"]
...
def import_mscoco(mode="train", is_mini=True, batch_size=100, num_epochs=1):
    ...
    x = iterator.get_next()
    image_id, input_seq, target_seq, indicator, image_features, object_features = (
        x["image_id"], x["input_seq"], x["target_seq"], x["indicator"], x["image_features"], x["object_features"])
