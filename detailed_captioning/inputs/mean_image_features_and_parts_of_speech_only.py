...
def _process_tf_record_proto(serialized_proto):
    ...
    image_id, caption = context["image/image_id"], sequence["image/caption_ids"]
    ...
    return {"image_id": image_id, "caption": caption, 
            "image_features": image_features, "parts_of_speech": parts_of_speech}


def _mask_and_slice(x):
    image_id, caption, image_features, parts_of_speech = (
        x["image_id"], x["caption"], x["image_features"], x["parts_of_speech"])
...
def _prepare_final_batch(x):
    (image_id, image_features, input_seq, target_seq, indicator, 
     pos_input_seq, pos_target_seq, pos_indicator) = (
        x["image_id"], x["image_features"], 
...
def import_mscoco(mode="train", is_mini=True, batch_size=32, num_epochs=1):
    ...
    padded_shapes = {"image_id": [], "image_features": [7 * 7 * 2048], 
                     "input_seq": [None], "target_seq": [None], 
                     "indicator": [None], 
                     "pos_input_seq": [None], "pos_target_seq": [None], 
                     "pos_indicator": [None]}
