...
Builds TF Records for the MSCOCO 2017 dataset for training an attribute detector'''
...
DatasetMetadata = namedtuple("DatasetMetadata",
                           ["image_id", "image_features", "attributes"])
...
def _to_sequence_example(image):
...
def _process_image_files(thread_index, ranges, name, images, attribute_map, 
                         num_shards, run_model_fn):
    ...
    Args:
        ...
        attribute_map: A Vocabulary object for the attribute names.
    ...
    for s in xrange(num_shards_per_batch):
        ...
        these_images = _preextract_dataset(these_images, attribute_map, run_model_fn)
        
        for image in these_images:
            sequence_example = _to_sequence_example(image)
...
def _process_dataset(name, images, attribute_map, num_shards, run_model_fn):
    ...
    for thread_index in xrange(len(ranges)):
        args = (thread_index, ranges, name, images, attribute_map, num_shards, run_model_fn)
...
def _preextract_batch(batch_of_images, attribute_map, run_model_fn):
    ...
    image_features = run_model_fn(np.stack(tensors))
    output_dataset = [
        DatasetMetadata(image_id=image.image_id, image_features=image_features[j, ...], 
                        attributes=attribute_map.sentence_to_attributes(image.captions[0])) 
        for j, image in enumerate(images)]
    return output_dataset


def _preextract_dataset(dataset_of_images, attribute_map, run_model_fn):
    """Runs the box extractor model on a single batch of images.
    Args:
    ...
    for i, image in enumerate(dataset_of_images):
        ...
        if len(batch_of_images) % BATCH_SIZE == 0:
            ...
            output_dataset += _preextract_batch(batch_of_images, attribute_map, run_model_fn)
            batch_of_images = []
...
def main(unused_argv):
    ...
    with tf.Session() as sess:
        ...
        _process_dataset("train", train_dataset, attribute_map, FLAGS.train_shards, run_model_fn)
        _process_dataset("val", val_dataset, attribute_map, FLAGS.val_shards, run_model_fn)
        _process_dataset("test", test_dataset, attribute_map, FLAGS.test_shards, run_model_fn)
